...
#define GOAL_FB_OFFSET -1.7
#define TURN_MULTIPLIER 1.6
#define ALIGN_OFFSET 71.5
...
BallFollower::BallFollower()
{
	...
	m_KickBallMaxCount = 5; // was 10
	m_KickBallCount = 0;

	m_KickTopAngle = -5.0;
	m_KickRightAngle = ALIGN_OFFSET - 3.0;
	m_KickLeftAngle = ALIGN_OFFSET + 3.0;

	m_FollowMaxFBStep = 30.0;
    m_FollowMinFBStep = 5.0;
	m_FollowMaxRLTurn = 10.0;
	m_FitFBStep = 3.0;
	m_FitMaxRLTurn = 35.0;
	m_UnitFBStep = 0.3;
	m_UnitRLTurn = 1.0;
...
void BallFollower::Process(Point2D ball_pos)
{
    ...
    else
    {
        ...
        fprintf(stderr, "Current PAN angle: %f KickBallCount: %d\n", pan, m_KickBallCount);
		double pan_range = Head::GetInstance()->GetLeftLimitAngle();
		double pan_percent = TURN_MULTIPLIER * (pan - ALIGN_OFFSET) / pan_range;
		...
		{
            fprintf(stderr, "m_KickRightAngle: %f m_KickLeftAngle: %f\n", m_KickRightAngle, m_KickLeftAngle);
			//if(tilt <= (tilt_min + MX28::RATIO_VALUE2ANGLE))
			//{
			//	if(ball_pos.Y < m_KickTopAngle)
			//	{
					fprintf(stderr, "Maybe I could shoot.");
					m_GoalFBStep = 0;
					m_GoalRLTurn = 0;
		...
		else
		{
			m_KickBallCount = 0;
			KickBall = 0;
			m_GoalFBStep = GOAL_FB_OFFSET;
            m_GoalRLTurn = m_FollowMaxRLTurn * pan_percent - 1.0;
			if(DEBUG_PRINT == true)
				fprintf(stderr, "[FOLLOW(P:%.2f T:%.2f>%.2f]", pan, tilt, tilt_min);
		}		
	}
