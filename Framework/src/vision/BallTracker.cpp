...
int BallTracker::SearchAndTracking(Point2D pos)
{
    ...
    if(pos.X < 0 || pos.Y < 0)
    {
        ...
        else
        {
            ...
            fprintf(stderr, "TiltTop: %f TiltBot: %f Pan: %f\n", TiltTopLimit, TiltBottomLimit, PanLimit);
            //printf("Tilt: %f\n", TiltTopLimit);
...
void BallTracker::Process(Point2D pos)
{
	...
	else
	{
		...
		Point2D offset = pos - center;
		offset *= -1; // Inverse X-axis, Y-axis
		offset.X *= 0.45*(Camera::VIEW_H_ANGLE / (double)Camera::WIDTH); // pixel per angle
		offset.Y *= 0.45*(Camera::VIEW_V_ANGLE / (double)Camera::HEIGHT); // pixel per angle
		ball_position = offset;
		Head::GetInstance()->MoveTracking(ball_position);
	}
}
